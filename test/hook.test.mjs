...
// The async hook hands CommonJS straight back so Node loads it through the ordinary CJS
// loader, where the `_compile` patch transforms it. Supplying `source` here would
// instead evaluate the module on the synchronous require(esm) bridge, which throws
// ERR_VM_MODULE_LINK_FAILURE on Node < 24.12 whenever the module's top-level require()
// chain reaches ESM (nodejs/node#59666).
test('should not rewrite a cjs module even if it matches a subscriber', async (t) => {
  ...
  // `resolve` registered a transformer for this URL and the deferral path is the only
  // thing that can free it. A second load must not reach the freed transformer.
  const again = await esmLoaderRewriter.load(url.url, {}, nextLoad)
  assert.ok(!again.shortCircuit)
})

test('should not rewrite a cjs module whose source another loader already supplied', async (t) => {
  const { esmLoaderRewriter } = t.ctx
  const cjsPath = path.join(import.meta.dirname, './example-deps/lib/node_modules/pkg-1/foo.js')
...
// The next two tests lock in the result.format → transformer module_type
...
// The `format === 'commonjs'` half of the mapping lives with the sync hooks
// (see hook-sync.test.mjs); the async hook never transforms CommonJS.
